...
function CustomMail({ threadId, onClose }: any) {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      onClose(); // Close the CustomMail component on success
    } catch (err) {
  ...
  <div
    className="bg-gradient-to-r from-[#4B63DD] to-[#0524BFFC] px-6 py-2 rounded-md flex items-center cursor-pointer transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#4B63DD] focus:ring-opacity-50"
    onClick={handleSendReply}
  >
    <span className="text-white font-semibold">
      {loading ? "Sending..." : "Send"}
    </span>
    <FaCaretDown className="ml-4 text-white" />
  </div>
  {error && <div className="text-red-500 ml-8">{error}</div>}
